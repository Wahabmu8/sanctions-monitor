...
في "designations" ضع الأفراد/الكيانات المُدرَجين فعلياً خلال آخر 14 يوماً (استخرج الأسماء من البيانات الرسمية).
مهم جداً: أدرج بحد أقصى 12 عنصراً في "items" و 25 عنصراً في "designations"، واجعل حقول body/role/info موجزة جداً (جملة قصيرة).
روابط رسمية حقيقية. ترجمة عربية دقيقة. أكمل الكائن كاملاً بأقواس صحيحة. JSON صالح 100% فقط.`;

async function call() {
  const body = { model: MODEL, max_tokens: 32000,
...
function closeJson(str){ const stack=[]; let inStr=false, esc=false;
  for(let k=0;k<str.length;k++){ const c=str[k];
    if(inStr){ if(esc)esc=false; else if(c==='\\')esc=true; else if(c==='"')inStr=false; continue; }
    if(c==='"')inStr=true; else if(c==='{'||c==='[')stack.push(c); else if(c==='}'||c===']')stack.pop(); }
  let out=str; for(let k=stack.length-1;k>=0;k--) out += (stack[k]==='{' ? '}' : ']'); return out; }
function parseObj(t){ const s=t.indexOf("{"); if(s<0) throw new Error("no json");
  let str=t.slice(s); const e=str.lastIndexOf("}"); if(e>0) str=str.slice(0,e+1);
  try{ return JSON.parse(str); }catch(_){ return JSON.parse(closeJson(str)); } }
...
try {
  ...
  const items = vItems(obj.items).slice(0, 12);
  const des = vDes(obj.designations).slice(0, 30);
